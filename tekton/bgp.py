...
class IpPrefixList(object):
    def __init__(self, name, access, networks):
        ...
        self._types = tuple(list(self.nettype) + [str])
        assert isinstance(access, Access)
...
class MatchPeer(Match):
    def __init__(self, peer):
        assert peer == VALUENOTSET or isinstance(peer, str)
        self._match = peer
...
class MatchNextHop(Match):
    def __init__(self, next_hop):
        assert is_empty(next_hop) or isinstance(next_hop, str)
...
class ActionSetLocalPref(Action):
    def __init__(self, local_pref):
        ...
        assert is_empty(local_pref) or isinstance(int(local_pref), int)
...
class ActionSetPrefix(Action):
    def __init__(self, prefix):
        assert is_empty(prefix) or isinstance(prefix, str)
        self._value = prefix
...
class ActionSetPeer(Action):
    def __init__(self, peer):
        assert is_empty(peer) or isinstance(peer, str)
        self._value = peer
