...
def extract_all_next_hops(next_hop_map):
    ...
    for src in next_hop_map:
        for _, next_hop in next_hop_map[src].items():
...
def compute_propagation(graph, ordered_paths):
    ...
    def allow_path(node, segment, index, origin):
        ...
        tmp = (origin, segment)
        tmp = segment
        if segment not in dag._node[node]['paths']:
            dag._node[node]['paths'].add(tmp)
            dag._node[node]['order'][index].add(tmp)
        # Unblock it if it was blocked by another path in the requirement
        if segment in dag._node[node]['block']:
            dag._node[node]['block'].remove(tmp)
    ...
    def block_path(node, segment, origin):
        ...
        tmp = (origin, segment)
        tmp = segment
        if tmp not in dag._node[node]['block'] and \
                tmp not in dag._node[node]['paths']:
            dag._node[node]['block'].add(tmp)
