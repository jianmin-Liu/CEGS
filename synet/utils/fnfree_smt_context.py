...
class EnumType(object):
    ...
    def __init__(self, name, values, z3_ctx):
        ...
        # Ensure values is a list to avoid dict_keys issues
        self._concrete_values = list(values) if not isinstance(values, list) else values
        self.z3_ctx = z3_ctx
        self._sort, self._symbolic_values = z3.EnumSort(name, self._concrete_values,
                                                        ctx=self.z3_ctx)
    ...
    def get_symbolic_value(self, value):
        ...
        indexof = list(self._concrete_values).index(value)
    ...
    def get_concrete_value(self, var):
        ...
        if var not in self._symbolic_values:
            ...
            raise ValueError(err)
        indexof = list(self.symbolic_values).index(var)
        return list(self.concrete_values)[indexof]
...
class SolverContext(object):
    ...
    def create_enum_type(self, name, values):
        ...
        for value in values:
            ...
            for ename, etype in self._enum_types.items():
    ...
    def fresh_var_name(self, prefix=None):
        ...
        name = "%s%d" % (prefix, self._next_varnum.__next__())
    ...
    def fresh_constraint_name(self, prefix=None):
       ...
       """
        if not prefix:
            ...
            prefix = sanitize_smt_name(prefix)
        name = "%s%d" % (prefix, self._next_constnum.__next__())
        while name in self._vars:
            name = "%s%d" % (prefix, self._next_constnum.__next__())
        return name
    ...
    def constraints_itr(self):
        ...
        """
        for name, value in self._tracked.items():
    ...
    def check(self, solver, track=True, set_model=True, out_smt=None):
        ...
        tracked_eq = []
        for name, func in self._enum_compare.items():
    ...
    def create_context(announcements, prefix_list=None, peer_list=None,
        ...
        origin_list = list(BGP_ATTRS_ORIGIN.__members__.keys())
        ...
        ctx.communities = list(announcements[0].communities.keys())
