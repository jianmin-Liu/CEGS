...
def get_propagated_info(propagation_graph, node,
    ...
    if not propagation_graph.has_node(node):
        return all_props
    for net, data in propagation_graph._node[node]['nets'].items():
...
class BGP(object):
    ...
    def compute_exported_routes(self):
        ...
        for peer, props in exported_info.items():
            self.log.debug("Node %s Exported to %s: %s", self.node, peer, props)

        # Second,  map the propagated to the local announcements
        export_anns = {}
        for neighbor, propagated in exported_info.items():
            n_attrs = self.ibgp_propagation._node[neighbor]
        ...
        for neighbor, vals in export_anns.items():
            # Since the announcements will change
            # We try to keep the ordering
            props = []
            anns = []
            for prop, ann in vals.items():
                props.append(prop)
                anns.append(ann)
    ...
    def compute_imported_routes(self):
        ...
        for neighbor in self.network_graph.get_bgp_neighbors(self.node):
            ...
            if self.node not in neighbor_exported:
                ...
                continue
            imported = {}
            for prop, ann in neighbor_exported[self.node].items():
            ...
            if rmap_name:
                ...
                props = []
                anns = []
                for prop, ann in imported.items():
                    props.append(prop)
                    anns.append(ann)
                tmp = self.anns_ctx.create_new(anns, self.compute_exported_routes)
                smt_map = SMTRouteMap(rmap, tmp, self.ctx)
                self.rmaps[rmap_name] = smt_map
                smt_map.execute()
                cc = list(self.ctx._tracked.keys())[:]
                for index, prop in enumerate(props):
    ...
    def mark_selected(self):
        for propagated, ann in self.anns_map.items():
    ...
    def synthesize(self, use_igp=False):
        ...
        anns_order = {}
        for net, info in self.ibgp_propagation._node[self.node]['nets'].items():
            if net not in anns_order:
                anns_order[net] = []
            anns_order[net] = info['order_info']

        for ann_name, values in anns_order.items():
            if len(values) == 1:
