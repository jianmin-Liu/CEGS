...
class EBGPVerify(object):
    ...
    def _extract_peering_graph(self):
        ...
        for asnum, routers in ases.items():
    ...
    def check_order(self, graph):
        ...
        for node in graph.nodes():
            graph._node[node]['order'] = [x for x in graph._node[node]['order'] if x]
        for node in graph.nodes():
            preds = set(flatten(flatten(graph._node[node]['order'])))
            for pred in preds:
                if pred == node:
                    continue
                # print "\t\tIn pred", pred
                if node in flatten(flatten(graph._node[pred]['order'])):
                    segment = self._get_segment(graph._node[node]['order'], pred, node)
                else:
                    segment = self._get_segment(graph._node[node]['order'], pred, None)
                comp = graph._node[pred]['order'][0]
                if segment[0] in graph._node[pred]['order']:
                    first_match = graph._node[pred]['order'].index(segment[0])
                    comp = graph._node[pred]['order'][first_match:len(segment) + 1]
